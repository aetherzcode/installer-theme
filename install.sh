...
check_token() {
  ...
  if [ "$USER_TOKEN" = "Aetherzx17" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Weh Ke AETHER No 6285798045817${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/aetherzcode/installer-theme/raw/main/C2.zip")
        break
        ;;
      2)
